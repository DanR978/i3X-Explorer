...
- **Model Insights** — a full main-panel page (opened from a Home summary card) built around what an engineer asks a single source of truth: the **type atlas** (the schema the instances imply — per type: placement distribution, composition with cardinality, depth, naming pattern), **conventions** (norms phrased affirmatively — auto-generated modeling documentation), **deviations** (exceptions to strong norms, Wilson-ranked and *explained*: outliers grouped by where they actually sit), **splits** (types whose placement is legitimately two populations, shown as distributions), a **data-quality hub** (orphans / untyped / unused types / duplicate ids, all browsable), and **most-anomalous objects** (count of independent norms broken). Exact counts, no ML, every list virtualized and every row navigable
...
- **One engine, one report, two surfaces.** `computeInsightsReport(objects, objectTypes)` is pure; `getInsightsReport(objects, objectTypes, namespaceCount)` wraps it in a one-slot memo keyed on the store arrays' reference identity and also runs `computeModelStats`, so the Home card, the stat row, and the full page all read one computation — the page rendering after the card is a cache hit, and their numbers can never disagree. Measured ~105ms over 100k objects (the perf test prints it; one shared `scanCatalog` — index, dedupe, depths — feeds both computations instead of each building its own). Recomputes only when the catalog reference changes; the explorer store's `setNamespaces`/`setObjectTypes` skip content-identical writes so the 30s background poll doesn't invalidate it while you sit on Home or the insights page
...
- **Three norm families**: placement (`unusual-parent`), containment (`missing-child`, with per-child-type cardinality histograms — min/max/modal), and **naming** (`nameSignature` collapses letter runs→`A`, digit runs→`9`, keeps separators; "PMP-999"→"A-9"; a bare `A` matches any word and is suppressed as uninformative). Per-object **anomaly scores** count independent norms broken (≥`MIN_ANOMALY_SCORE` ranks, top-`TOP_ANOMALIES` via `topBy`), each point a named reason
- **The page** (`src/components/insights/InsightsView.tsx`) is a third main-panel state with the diff view's exact lifecycle: `stores/insights.ts` holds only `viewOpen`; module-level subscriptions close it on any navigation and on the diff view opening (`openView` closes the diff in return — last-opened wins; the import is one-way insights→diff, no cycle). Content survives closing (it lives in the report memo), so reopening is instant. Sections: summary tiles (click scrolls), the **type atlas** (the centerpiece — a windowed browsable inferred schema, NOT a node-link graph; stacked placement mini-bars are one hue with rank→opacity, labels in tooltips, never color-only), conventions, deviations (expandable, chips ≤8 else inner windowed lists, lazy copy-as-JSON via the shared `common/CopyJsonButton`), splits, the **data-quality hub** (orphans/untyped/unused/duplicate ids with fully browsable windowed lists — the Home *Worth knowing* strip stays as the quick signal), and most-anomalous objects. All lists window through `insights/WindowedList.tsx` (dynamic row heights via `measureElement`); all navigation goes through `useElementNavigation`
...
- **State lives in its own Zustand store** (`stores/diff.ts`), never the explorer store — loading a baseline leaves the tree completely untouched. `DiffView` is a main-panel state alongside Home and element detail; **any navigation closes it** (a store-level subscription watches selection changes — a diff row click, tree click, search, Back/Forward all leave the view), and the baseline + result stay loaded so the toolbar reopens it instantly. Row clicks go through `selectElement` — the same navigation contract as everything else; removed objects render from baseline data and are visibly non-navigable
