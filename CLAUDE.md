...
**Why the tree is the default.** The tree labels every node on its own row with no zooming required — the drawing grows to fit, so a 100-node depth-2 walk is a tall, scrollable, readable tree. The rings view is **semantic-zoom**: ring radii are fixed and compact (rings = hops; geometry never depends on content or label text), while dots, labels and strokes render at constant *screen* size and labels are culled by each node's weighted angular slot (`slot` on `radialLayout.ts` nodes) — hubs own wide slots and label first, zooming in reveals the rest, and each ring's caption carries its per-hop count ("1 hop · 1,240"). Dense edge fans get reduced opacity. That keeps the fitted view a readable overview at any fan-out, but reading *every* name still takes zooming, so rings stay the secondary view — a compact spatial sense of where things sit.
