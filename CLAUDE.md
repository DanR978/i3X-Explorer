...
- **`RelationshipList.tsx`** renders that tree, and **containment is the whole design problem**. A nested list that expands freely is worse than the flat one it replaced: a single container with 2,000 children pushes everything after it off the panel, and the nesting meant to show structure hides it. Three independent limits: **`planAutoExpand`** opens rows breadth-first until `AUTO_EXPAND_ROWS` (200) is spent, so a small model opens fully and matches the map 1:1 while a hub opens its top and leaves the rest behind counts (a branch too big to fit is *skipped*, not a full stop, so one giant node can't close the whole level); **`PAGE_SIZE`** (50) rows per level with a `Show N more` below the block; and each block past `MAX_BLOCK_ROWS` (8) scrolls inside a capped container whose bottom dissolves under a `mask-image` gradient, the mask lifts once you scroll to the end, since a permanently half-faded last row reads as a rendering bug. Manual open/collapse overrides the budget and is never undone by it; a new walk resets both, since the old plan described a shape that no longer exists. Every row at every depth keeps drag-to-focus, the ◎ button and hover-spotlight
...
- **Drag a row from the list onto the tree** to re-root it on that element *without navigating*: the detail view around it stays put, and **the list re-roots with it**, they share one walk, so the pane that shows the connections and the pane that draws them always describe the same element. "Back to <name>" in the header returns both. The drop payload is the `ELEMENT_DRAG_TYPE` MIME carrying an elementId, resolved against `objectIndex`; the ◎ button on each row does the same thing with the whole object in hand, and is the keyboard path. Clicking a tree node navigates (as on the overview map)
