...
- Relationships tab puts **the direct-relationship list and a depth-N map in one full-width panel** (Fusion 360 style: browser tree on the left, map on the right). The list shows every direct relationship (hierarchy and non-hierarchy together); the map walks out to a configurable depth (pills for 1 and 2, plus a "+" that reveals one deeper hop per click, default 1), rings are hops, and every node is labeled. Hover a list row to spotlight its node on the map, or drag a row onto the map to re-center it there without navigating away
...
### Relationships Tab (one panel: list + depth-N map)
The list and the map share a single card, laid out like a Fusion 360 workspace: the browser tree (every direct relationship) on the left, the map (the depth-N walk) on the right. The tab spans the full window width; the other detail tabs stay capped at 960px. The list answers *what is this connected to* (every direct relationship, hierarchy and non-hierarchy, in one place). The map answers *what is it connected to through those*, which is only a real question past the first hop, so it walks out to a configurable depth.
...
- **`radialLayout.ts`** is pure and deterministic, no simulation. Root at the center, one ring per hop, so *distance from the center IS the depth* (a force blob would put a 3-hop node anywhere). Angles come from a weighted sector allocation down the BFS tree (each node hands its children a slice of its own sector sized by subtree weight), so siblings stay together and tree edges never cross. **Every node is labeled**, and spacing is label-aware: each ring's radius grows with how many nodes it holds and the width of the widest label on it, and the gap between rings clears the widest label, so a crowded ring spreads out instead of stacking names. Non-tree edges are drawn as chords. `RelationshipGraph` clips long names to `MAX_LABEL_CHARS` (shared with the layout's width math); the hover card carries the full name
...
- **The panel fills the detail area's height** and grows/shrinks with the window, down to a `min-h-[30rem]` floor (below that the detail area scrolls); the map pane has a `min-w` floor too. The map SVG fits-and-centers (`meet`) so the whole graph stays visible
- **Hover a list row to spotlight its node on the map** (`externalHoverId`): the map highlights it exactly as a real map hover does, and the active node gets a focus ring so it is easy to find. A real map hover wins; a row whose node isn't drawn is ignored
- **Drag a row from the list onto the map** to re-center the map on it *without navigating*: the detail view around it stays put. The drop payload is the `ELEMENT_DRAG_TYPE` MIME carrying an elementId, resolved against `objectIndex`; the ◎ button on each row does the same thing with the whole object in hand, and is the keyboard path. Clicking a map node navigates (as on the overview map)
