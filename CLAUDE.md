...
- **Horizontal scroll**: tree labels use `whitespace-nowrap` (not `truncate`) so rows grow to natural width; the tree body is a both-axes scroll area with an inner `w-max min-w-full` wrapper (rows share a uniform width — highlights and count pills stay correct — and overflow horizontally). The filter input is a fixed header above the scroll body so it neither stretches nor scrolls away. The inner wrapper's min-width is `max(100%, measuredPx)` — an inline px min-width alone would override `min-w-full` and let rows fall short of the panel edge on narrow content
