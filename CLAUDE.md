...
- Relationships tab puts **the direct-relationship list and a depth-N tree in one full-width panel** (Fusion 360 style: browser list on the left, tree on the right). The list shows every direct relationship (hierarchy and non-hierarchy together); the tree walks out to a configurable depth (pills for 1 and 2, plus a "+" that reveals one deeper hop per click, default 1), drawn left-to-right with the root on the left, each hop a column, and every node on its own row so labels never overlap. Hover a list row to spotlight its node on the tree, or drag a row onto the tree to re-root it there without navigating away
...
**Why a left-to-right tree, not a radial map.** The map was radial (rings = hops) through several iterations, but a radial layout has two failure modes it can't shake: it reserves a full circle while real data uses a narrow wedge (so a sparse graph sits small and off-centre in empty space), and a deep chain stacks parent/child labels at the same angle (so names overlap no matter how they're sized). A left-to-right tree fixes both structurally — depth is the column, **every node gets its own row**, so a label always has space and the drawing grows to fit. It also scales: a 100-node depth-2 walk becomes a tall, scrollable, readable tree instead of a hairball.
...
- **`treeLayout.ts`** is pure and deterministic. It lays the BFS tree (each node's `via` is its parent) left-to-right: x is `depth * COLUMN_WIDTH`; a leaf takes the next row (`ROW_HEIGHT`), a parent is centred on its children. Recursion is bounded by depth (≤ `MAX_RELATIONSHIP_DEPTH`). It returns node/edge positions, the `depths` present (for the column guides), and the content bbox. Cross-links (non-tree edges) are flagged `tree:false` and drawn as dashed curves; tree edges are solid-ish S-curves in the bucket color
- **The view is the content bbox, clamped.** `RelationshipGraph` builds the `viewBox` from the tree's bounding box, clamped to `[MIN_VIEW, MAX_VIEW]`: a small tree shows centred at a comfortable size (not blown up), a huge one shows at a readable scale with the rest reached by scrolling. Labels and nodes are in diagram units and scale with the map — because each node owns a row, they never collide at any scale, so there's no semantic-zoom/decluttering machinery. Long names clip to `MAX_LABEL_CHARS`; the hover card carries the full name
