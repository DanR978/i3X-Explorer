...
- Tree micro-toolbar next to the filter input: collapse-all and reveal-selection buttons
- Tree visuals: VS Code-style indent guides at every depth (the selected node's subtree guide is accented), counts as rounded pills, newly expanded children ease in (disabled under reduced motion), and hovering a row reveals a copy-element-ID action (copy-URI on namespaces)
- Tree scrolls horizontally for long/deeply-nested labels
...
- The tree is split into focused modules under `src/components/tree/`: `treeData.ts` (non-JSX shared logic — `buildTreeRows`, `resolveCompositionFlags`, `refreshAllObjects`, `hasCompositionChildren`, `getObjectLabel`, `isScalarSchemaType`, folder-id/`MAX_TREE_DEPTH` constants), `TreeNode.tsx` (the row renderers: `TreeNode`, `TreeMoreNode`, `TreeRowIcon`), and `TreeView.tsx` (the shell: virtualizer, sticky header, width/height measurement, background refresh). Dependencies point one way into `treeData`/`TreeNode`, so there is no import cycle.
- **The whole tree is one flattened, virtualized list.** `buildTreeRows` (pure, memoized) walks all three folders — Namespaces → types → objects, the flat Objects forest, and the Hierarchy parent/child walk — applying expansion, filtering (with ancestor keep-alive), per-parent paging, and cycle/depth guards in a single pass, and emits one ordered `TreeRow[]` where each row carries its `parentIndex`. `TreeView` windows that array with a single `@tanstack/react-virtual` virtualizer against the tree scroll container. Nothing renders recursively: a hierarchy node with 20k children costs the same DOM as one with 2. Rows render in normal flow with top/bottom spacer padding (not absolute positioning) so horizontal scroll, hover, selection, and count pills behave like plain DOM; row height is measured once from the first mounted row; content width comes from an approximate pass over all rows that exact-measures only the ~48 widest candidates with canvas text metrics. Reveal-selected is pure index math (`scrollToIndex`), no DOM probing. Expansion state lives in the store, so expanded rows survive being scrolled out of the window.
...
- **Horizontal scroll**: tree labels use `whitespace-nowrap` (not `truncate`) so rows grow to natural width; the tree body is a both-axes scroll area with an inner `w-max min-w-full` wrapper (rows share a uniform width — highlights and count pills stay correct — and overflow horizontally). The filter input + micro-toolbar is a fixed header above the scroll body so it neither stretches nor scrolls away
