...
- **Hierarchy / Relations toggle** at the top of the sidebar: the third folder holds the same roots either way, but nests them either by `parentId` (one parent per node, free, from the store) or by **every relationship the server reports**, grouped by kind. In Relations mode a node opens into `Has parent · Has component 640 · Supplies to 2 · Monitored by 1`, so a "feeds" edge is at a fixed position under the node instead of buried behind 640 components
...
- **The third folder has two walks, and a `Hierarchy / Relations` toggle picks one** (`treeStructure` in the store; `relationshipTree.ts` is the pure half, `walkRel` in `treeData.ts` the walk). Same `hierarchicalRoots` either way, so toggling never moves the top level, and each mode keeps its own expansion (the `hier:` and `rel:` id namespaces are disjoint and both just stay in `expandedNodes`), so switching over to check a connection and switching back lands you where you were. The design points that matter:
  - **A row's id is its PATH, not its element** (`rel:` + elementIds joined by NUL, see `REL_SEP`). The hierarchy is a tree by construction; relationships are a graph, where one hub legitimately hangs off many places at once. Keying expansion on the element would open every occurrence of it simultaneously, which on a densely linked model is an explosion. Ancestors are then exactly path prefixes, which is what `expandedPathTo` and the paging exemption both use
  - **A row never repeats an object already on its own branch.** Dropping only the edge you arrived through is not enough and looks fine until the walk is two hops deep: *every* descendant carries a `HasParent` back up, so the root reappears as a dead leaf under every node beneath it (three copies of `pump-station` in a twelve-row screenshot). Filtering against the whole path kills that, terminates loops without a `(cycle detected)` marker, and keeps the genuinely new sideways edges
  - **Neighbours are grouped by relationship type, not flattened with a badge per row.** Badges are denser and read fine on a node with eight neighbours, but a pump with 600 components and two `SuppliesTo` edges buries the two rows you switched views to find, behind a paging control. A group header puts every kind at a fixed position under the node regardless of fan-out, and carries its own count, its own collapse and its own paging. **A node with a single kind draws no header at all**: an indent level charged for nothing
  - **Group labels are humanized, including the declared ones** (`relationshipLabel`). `/relationshiptypes` is fetched on connect (it was dead code before this), but servers overwhelmingly declare `displayName` as the identifier itself, so taking it verbatim just puts `HasParent` in a column of prose. Everything goes through `humanizeRelationship` → "Has parent", "Supplied by"; a server that already writes prose passes through unchanged. Sentence case, and **not** CSS-uppercased, which would throw that away
  - **Expanding a node costs one request** (`POST /objects/related`, no type filter) — unlike the hierarchy walk, which is pure store data. Chevrons are optimistic until `resolveRelationshipFlags` resolves the on-screen rows in one batch (same contract as `resolveCompositionFlags`), caching neighbour *ids* rather than a count, because the count a row shows depends on where it hangs. `planGroupExpansion` opens a small node's groups on expand and leaves a big one closed
  - **Known limit**: the sidebar filter only matches rows already fetched, since an unexpanded relationship branch is genuinely unknown; the roots keep the hierarchy's `parentId` keep-alive. And an external reveal (search, a diff or insights row, `selectElement`) forces the folder back to Hierarchy — the only walk that can place an arbitrary object, since a relationship walk places it at however many paths reach it, possibly none currently open. The tree's own rows never come through there
